...
std::vector<std::vector<float> > removing_edge(temp_threed_object temp_threed,fts twod){
    ...
    std::vector<std::vector<float> >  corrected_edges;
    std::vector<std::vector<float> > temp_threed_edge ;

    temp_threed_edge =temp_threed.edge;  // all possible edges of  3D;

    std::vector<std::vector<float> > f_edge=twod.fedge;
    std::vector<std::vector<float> > t_edge=twod.tedge;
    std::vector<std::vector<float> > s_edge=twod.sedge;

    float x1,y1,x2,y2;


    // for (int g=0 ;g< s_edge.size();g++){
    // 			std::vector<float> vec= s_edge.at(g);
    ...
    for(int i=0;i<temp_threed_edge.size();i++){
        ...
        // std::vector<std::vector<float> > tempo;
        // tempo = temp_threed.edge;


        std::vector<float> temp_edge;
        ...
        //check in topview
        std::vector<std::vector<float> > check_t;
        check_t= new_twod.tedge;



        std::vector<float> check_t_v;
        ...
        // 		std::vector<float> vec= check_t.at(g);
        // 		for (int b=0;b<vec.size();b++){
        // 			std::cout << vec.at(b);
        // 		}
        // 		std::cout << "\n";
        ...
        //check in frontview
        std::vector<std::vector<float> > check_f;
         check_f= new_twod.fedge;
        std::vector<float> check_f_v ;
        ...
        // 		std::vector<float> vec= check_f.at(g);
        // 		for (int b=0;b<vec.size();b++){
        // 			std::cout << vec.at(b);
        // 		}
        // 		std::cout << "\n";
        ...
        //check in sideview
        std::vector<std::vector<float> > check_s ;
        check_s = new_twod.sedge;
        std::vector<float> check_s_v;
        ...
        // 		std::vector<float> vec= check_s.at(g);
        // 		for (int b=0;b<vec.size();b++){
        // 			std::cout << vec.at(b);
        // 		}
        // 		std::cout << "\n";
        ...
        // 		std::vector<float> vec= corrected_edges.at(g);
        // 		for (int b=0;b<vec.size();b++){
        // 			std::cout << vec.at(b);
        // 		}
        // 		std::cout << "\n";
